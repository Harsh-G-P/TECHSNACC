        ...
        <div className="flex items-center gap-5">
          ...
          {userData ? (
            <div className="relative" ref={dropdownRef}>
              <div
                className="w-8 h-8 flex justify-center items-center bg-black text-white rounded-full cursor-pointer"
                onClick={() => setDropdownOpen((prev) => !prev)} // 👈 toggle on click
              >
                {userData.name?.[0]?.toUpperCase()}
              </div>
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 bg-white shadow-md rounded-md py-2 w-40 z-50 text-sm text-gray-600">
                  <span
                    onClick={() => {
                      navigate("/profile");
                      setDropdownOpen(false);
                  ...
                  <span
                    ...
                    className="block px-4 py-2 text-red-500 hover:bg-red-50 cursor-pointer"
                  >
                    Logout
                  </span>
                </div>
